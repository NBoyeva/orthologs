...
for file in files:
	...
	records = list(SeqIO.parse(open(os.path.join(data_dir, file)), 'fasta'))
	regex = re.compile("\[gene=[\w]*\]")  # \w: [a-zA-Z0-9_]
	
	# a list to track if we already met specific gene name (length equals to the number of unique gene names)
	gene_names = []
	# a list to build filter list further (length equals to the number of records) 
	names = [] 

	for record in records: # records are all records from fasta file 
